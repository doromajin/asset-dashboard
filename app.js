...
(function () {
  ...
  var FUNDS = [
    {name:'eMAXIS Slim 全世界株式', short:'オルカン', units:0, nav:20000, monthly:20000, costBasis:0},
    {name:'iFreeNEXT FANG+', short:'FANG+', units:0, nav:30000, monthly:20000, costBasis:0}
  ];
  ...
  function buildFundInputs() {
    ...
    FUNDS.forEach(function (f, i) {
      ...
      row.innerHTML =
        ...
        '<input type="number" data-idx="' + i + '" data-field="costBasis" value="' + f.costBasis + '" step="1000" placeholder="累計取得額">';
      c.appendChild(row);
    });
  }
  ...
  function loadData() {
    try {
      ...
      if (d.funds) d.funds.forEach(function (f, i) { if (FUNDS[i]) { FUNDS[i].units = f.units; FUNDS[i].nav = f.nav; FUNDS[i].monthly = f.monthly; FUNDS[i].costBasis = f.costBasis || 0; } });
  ...
  function saveData() {
    try {
      var d = {
        ...
        funds: FUNDS.map(function (f) { return {units: f.units, nav: f.nav, monthly: f.monthly, costBasis: f.costBasis}; }),
  ...
  function applyContribution() {
    syncFromInputs();
    var s = scalars();
    FUNDS.forEach(function (f) { if (f.nav > 0 && f.monthly > 0) { f.units += f.monthly / f.nav * 10000; f.costBasis = (f.costBasis || 0) + f.monthly; } });
  ...
  // 積立ステップ変更管理
  var tsumiSteps = []; // [{year, amount}]
  var MAX_STEPS = 3;

  window.addTsumiStep = function () {
    if (tsumiSteps.length >= MAX_STEPS) return;
    // デフォルト: 既存ステップの最大年+1 or 2年後
    var defaultYear = tsumiSteps.length > 0 ? Math.min(tsumiSteps[tsumiSteps.length - 1].year + 1, 9) : 2;
    tsumiSteps.push({year: defaultYear, amount: 60000});
    renderTsumiSteps();
    calcSim();
  };

  window.removeTsumiStep = function (idx) {
    tsumiSteps.splice(idx, 1);
    renderTsumiSteps();
    calcSim();
  };

  function renderTsumiSteps() {
    var container = $('sim-tsumi-steps');
    if (!container) return;
    var addBtn = $('sim-add-step');
    if (addBtn) addBtn.style.display = tsumiSteps.length >= MAX_STEPS ? 'none' : 'block';

    container.innerHTML = '';
    tsumiSteps.forEach(function (step, idx) {
      var div = document.createElement('div');
      div.style.cssText = 'background:var(--bg-tertiary); border-radius:6px; padding:10px 10px 8px; margin-bottom:6px;';
      ...
      yearSelect.style.cssText = 'background:var(--bg-secondary); color:var(--text-primary); border:1px solid var(--border); border-radius:4px; padding:2px 6px; font-size:13px;';
      for (var y = 1; y <= 10; y++) {
        var opt = document.createElement('option');
        opt.value = y;
        opt.textContent = y + '年後から';
        if (y === step.year) opt.selected = true;
        yearSelect.appendChild(opt);
      }
      (function(i) {
        yearSelect.addEventListener('change', function () {
          tsumiSteps[i].year = parseInt(this.value);
          calcSim();
        });
      })(idx);

      var amtLabel = document.createElement('span');
      amtLabel.style.cssText = 'font-size:13px; font-weight:500; color:var(--text-primary);';
      amtLabel.textContent = fmtYen(step.amount);

      var delBtn = document.createElement('button');
      delBtn.textContent = '✕';
      delBtn.style.cssText = 'background:none; border:none; color:var(--text-tertiary); font-size:14px; cursor:pointer; padding:0 0 0 8px; line-height:1;';
      (function(i) {
        delBtn.addEventListener('click', function () { window.removeTsumiStep(i); });
      })(idx);

      var left = document.createElement('div');
      left.style.cssText = 'display:flex; align-items:center; gap:8px;';
      left.appendChild(yearSelect);
      left.appendChild(amtLabel);
      ...
      slider.type = 'range';
      slider.min = 0; slider.max = 200000; slider.step = 5000;
      slider.value = step.amount;
      slider.style.width = '100%';
      (function(i, lbl) {
        slider.addEventListener('input', function () {
          tsumiSteps[i].amount = parseInt(this.value);
          lbl.textContent = fmtYen(tsumiSteps[i].amount);
          calcSim();
        });
      })(idx, amtLabel);

      var hint = document.createElement('div');
      hint.style.cssText = 'display:flex; justify-content:space-between; font-size:11px; color:var(--text-tertiary); margin-top:2px;';
      hint.innerHTML = '<span>¥0</span><span>¥10万</span><span>¥20万</span>';
  ...
  window.openSim = function () {
    ...
    // レイアウト確定後にcalcSim（canvasのclientWidthが正しく取れるよう）
    setTimeout(calcSim, 0);
  };
  ...
  function calcSim() {
    ...
    var stockLabelEl = $('sim-stock-label');
    var fundLabelEl = $('sim-fund-label');
    var dtLabelEl = $('sim-dt-label');
    var tsumiLabelEl = $('sim-tsumi-label');
    if (!stockLabelEl) return; // モーダルが閉じてたら即リターン
    stockLabelEl.textContent = (stockRate * 100).toFixed(0) + '%';
    fundLabelEl.textContent = (fundRate * 100).toFixed(0) + '%';
    dtLabelEl.textContent = fmtYen(dtMonthly);
    tsumiLabelEl.textContent = fmtYen(tsumiMonthly);
    ...
    // NISA枠の取得額(簿価)管理
    var NISA_TSUMI_MAX = 18000000;  // 積立投資枠 生涯上限(つみたて枠だけで埋める場合の上限)
    var NISA_GROWTH_MAX = 12000000; // 成長投資枠 生涯上限
    var NISA_TOTAL_MAX = 18000000;  // 合計生涯上限
    var nisaTsumiCost = 0;
    FUNDS.forEach(function (f) { nisaTsumiCost += f.costBasis || 0; });
    var nisaGrowthCost = 0;
    STOCKS.forEach(function (st) { nisaGrowthCost += (st.cost || 0) * st.shares * s.fx; });
    var nisaCostArr = [Math.min(nisaTsumiCost, NISA_TSUMI_MAX) + Math.min(nisaGrowthCost, NISA_GROWTH_MAX)];
    ...
    for (var y = 1; y <= 10; y++) {
      // ステップ変更: このyearから積立額が変わる設定があれば適用（複数あれば最後のものを優先）
      var tsumi = tsumiMonthly;
      tsumiSteps.forEach(function (step) {
        if (y >= step.year) tsumi = step.amount;
      });
      stocks = stocks * (1 + stockRate) + dtMonthly * 12;
      funds = funds * (1 + fundRate) + tsumi * 12;
      ide = ide * (1 + fundRate);
      stockArr.push(Math.round(stocks));
      fundArr.push(Math.round(funds + ide));
      totalArr.push(Math.round(stocks + funds + ide + cash));

      // NISA取得額の累計（積立枠は年120万で頭打ち、超過分は成長枠扱い）
      var tsumiAnnual = Math.min(tsumi * 12, 1200000);
      var overflow = Math.max(0, tsumi * 12 - 1200000);
      nisaTsumiCost = Math.min(nisaTsumiCost + tsumiAnnual, NISA_TSUMI_MAX);
      nisaGrowthCost = Math.min(nisaGrowthCost + overflow + dtMonthly * 12, NISA_GROWTH_MAX);
      var nisaCostTotal = Math.min(nisaTsumiCost + nisaGrowthCost, NISA_TOTAL_MAX);
      nisaCostArr.push(Math.round(nisaCostTotal));
    }
    ...
    // 生年月日(2005/06/27)から、25歳到達・30歳到達までの年数を動的算出
    var BIRTH = new Date(2005, 5, 27); // 月は0始まり
    var now = new Date();
    var age = now.getFullYear() - BIRTH.getFullYear();
    var hadBirthdayThisYear = (now.getMonth() > BIRTH.getMonth()) ||
      (now.getMonth() === BIRTH.getMonth() && now.getDate() >= BIRTH.getDate());
    if (!hadBirthdayThisYear) age--;
    // 今後何回目の誕生日で26歳・31歳になるか(その前年=25歳/30歳の間が目標期限)
    var goalYear25 = 26 - age;
    var goalYear30 = 31 - age;

    var tableHTML = '';
    // ヘッダー行
    tableHTML += '<div style="display:flex; justify-content:space-between; align-items:center; padding:0 0 6px; border-bottom:0.5px solid var(--border);">' +
      '<span style="font-size:11px; color:var(--text-tertiary);"></span>' +
      '<span style="display:flex; gap:10px;"><span style="font-size:11px; color:var(--text-tertiary); width:90px; text-align:right;">評価額</span><span style="font-size:11px; color:var(--text-tertiary); width:90px; text-align:right;">NISA取得額</span></span>' +
      '</div>';
    // 現在行
    tableHTML += '<div style="display:flex; justify-content:space-between; align-items:center; padding:6px 0; border-bottom:0.5px solid var(--border); background:rgba(167,139,250,0.08);">' +
      '<span style="color:var(--purple); font-size:13px;"><b>現在</b></span>' +
      '<span style="display:flex; gap:10px;"><span style="font-weight:500; color:var(--purple); width:90px; text-align:right; font-size:12px;">' + fmtYen(cur) + '</span><span style="color:var(--text-tertiary); width:90px; text-align:right; font-size:12px;">' + fmtYen(nisaCostArr[0]) + '</span></span>' +
      '</div>';
    for (var i = 1; i <= 10; i++) {
      var t = totalArr[i];
      var isGoal25 = i === goalYear25, isGoal30 = i === goalYear30;
      var GOAL = (goalYear30 >= 1 && i >= goalYear30) ? GOAL_30 : GOAL_25;
      ...
      tableHTML += '<div style="display:flex; justify-content:space-between; align-items:center; padding:6px 0; border-bottom:0.5px solid var(--border);' + rowBg + '">' +
        '<span style="color:' + (isGoal25 || isGoal30 ? 'var(--purple)' : 'var(--text-secondary)') + '; font-size:13px;">' + labelStr + '</span>' +
        '<span style="display:flex; gap:10px; align-items:baseline;">' +
          '<span style="width:90px; text-align:right;"><span style="font-weight:500; color:' + c + '; font-size:12px;">' + fmtYen(t) + '</span><br><span style="font-size:10px; color:var(--text-tertiary);">(' + g + '%)</span></span>' +
          '<span style="width:90px; text-align:right; color:var(--text-tertiary); font-size:12px;">' + fmtYen(nisaCostArr[i]) + '</span>' +
        '</span>' +
        '</div>';
    }
  ...
  window.addEventListener('load', function () {
    ...
    // 途中変更ボタンにタッチイベント追加(スマホ対応)
    var addBtn = document.getElementById('sim-add-step');
    if (addBtn) {
